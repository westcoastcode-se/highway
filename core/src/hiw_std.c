...
bool hiw_memory_resize_enabled(hiw_memory* m)
{
	return m->resize_bytes <= 0;
}

int hiw_memory_size(hiw_memory* const m)
{
	return (int)(m->pos - m->ptr);
}

int hiw_memory_capacity(hiw_memory* const m)
{
	return (int)(m->end - m->ptr);
}
...
void hiw_memory_reset(hiw_memory* m)
{
	m->pos = m->ptr;
}
